...
export default {
  ...
  methods:{
    handleSend(message, next, file) {
      if (message.type=="text"){
        axios({
          ...
          method:'post',
          data:{
            UserID:message.toContactId,
            StoreID:this.$store.state.MerchantsID,
    ...
    handlePullMessages(contact, next) {
      ...
      axios.get(`http://localhost:8080/demo/api/Messages/GetMessage?BusinessID=${this.$store.state.MerchantsID}&UserID=${contact.id}`).then(
          response=> {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:8080/demo/api/Messages/GetUsersInfo?MerchantsID=${this.$store.state.MerchantsID}`).then(
        response=>{
          let merchants = response.data;
          merchants.forEach((item)=>{
            this.contacts.push({
              id: item.UserID,
              displayName: item.UserAccount,
    ...
    //查询当前用户
    axios.get(
      `http://localhost:8080/demo/api/Merchants/Merchants?MerchantsID=${this.$store.state.MerchantsID}`,
    )
        .then(
            response=>{
              this.user={id: this.$store.state.MerchantsID, displayName: response.data.BusinessAccount, avatar:"http://localhost:61124"+response.data.BusinessImg}
            },
            error=>{
              console.log(error.message)
            }
