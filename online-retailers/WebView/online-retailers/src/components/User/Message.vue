...
export default {
  ...
  mounted() {
    ...
    axios.get(`http://localhost:8080/demo/api/Messages/GetMerchantsInfo?UserID=${this.$store.state.userId}`).then(
        response=>{
          let merchants = response.data;
          merchants.forEach((item)=>{
            this.contacts.push({
              id: item.BusinessID,
              displayName: item.BusinessAccount,
              avatar: "http://localhost:61124" + item.BusinessImg,
              index: 'L',
              unread: 0,
